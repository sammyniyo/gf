...
<div class="max-w-4xl space-y-6">
    ...
    <div class="glass-card p-6">
        <form method="POST" action="{{ route('admin.stories.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category') }}" placeholder="e.g., Testimony, Event"
                           class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-1 focus:ring-indigo-400">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="excerpt" rows="2" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-1 focus:ring-indigo-400">{{ old('excerpt') }}</textarea>
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Content *</label>
                    <textarea name="content" rows="8" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-1 focus:ring-indigo-400">{{ old('content') }}</textarea>
                </div>
                <div>
