    ...
    <form action="{{ route('admin.albums.update', $album) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center space-x-2">
                ...
                <span id="cover_image_filename" class="text-gray-400 dark:text-gray-300">
                    {{ $album->cover_image ? basename($album->cover_image) : 'No file chosen' }}
                </span>
                <input type="file"
                       id="cover_image"
                       name="cover_image"
                       ...
                       class="hidden"
                       onchange="document.getElementById('cover_image_filename').textContent = this.files[0] ? this.files[0].name : '{{ $album->cover_image ? basename($album->cover_image) : 'No file chosen' }}';">
            </div>
        ...
        <div class="mb-6 p-6 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                ...
                Album ZIP File {{ $album->download_link ? '(Replace existing)' : '' }}
            </h3>

            @if($album->download_link)
            <div class="mb-4 p-3 bg-green-100 dark:bg-green-800/30 rounded-lg border border-green-200 dark:border-green-700">
                <div class="flex items-center">
                    <svg class="w-4 h-4 text-green-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="text-sm font-medium text-green-800 dark:text-green-200">ZIP file already uploaded</span>
                </div>
                <p class="text-xs text-green-700 dark:text-green-300 mt-1">Current file: {{ basename($album->download_link) }}</p>
            </div>
            @endif
            ...
            <div class="flex items-center space-x-2">
                ...
                <span id="zip_file_filename" class="text-gray-400 dark:text-gray-300">
                    {{ $album->download_link ? basename($album->download_link) : 'No file chosen' }}
                </span>
                <input type="file"
                       id="zip_file"
                       name="zip_file"
                       accept=".zip"
                       class="hidden"
                       onchange="document.getElementById('zip_file_filename').textContent = this.files[0] ? this.files[0].name : '{{ $album->download_link ? basename($album->download_link) : 'No file chosen' }}';">
            </div>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">ZIP file only. Max 100MB. Leave empty to keep current file.</p>
            @error('zip_file')
            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
...
<script>
(function() {
    const form = document.querySelector('form[action="{{ route('admin.albums.update', $album) }}"]');
    if (!form) return;
    ...
    form.addEventListener('submit', function(e) {
        // Track only when files are being uploaded
