                            ...
                            <div class="relative">
                                <a href="{{ route('devotions.show', $devotion) }}" class="block">
                                    @if($devotion->featured_image)
                                        <div class="relative h-64 overflow-hidden">
                                            <img src="{{ asset('storage/' . $devotion->featured_image) }}" alt="{{ $devotion->title }}" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110">
                                ...
                                <div class="p-6 space-y-4">
                                    <h3 class="text-2xl font-bold text-gray-900 line-clamp-2 leading-tight group-hover:text-emerald-600 transition-colors">
                                        <a href="{{ route('devotions.show', $devotion) }}">{{ $devotion->title }}</a>
                                    </h3>
                                    ...
                                    @if($devotion->scripture_reference)
                                        <div class="flex items-center gap-2 text-emerald-700">
                                            ...
                                            <span class="text-sm font-semibold">{{ $devotion->scripture_reference }}</span>
                                        </div>
                                    @endif

                                    <p class="text-gray-600 line-clamp-3 leading-relaxed">{{ $devotion->excerpt }}</p>
                                    ...
                                    <div class="flex items-center justify-between border-t-2 border-gray-100 pt-4">
                                        @if($devotion->author)
                                            <div class="flex items-center gap-2 text-sm text-gray-500">
                                                ...
                                                <span class="font-medium">{{ $devotion->author }}</span>
                                            </div>
                                        @endif
                                        <a href="{{ route('devotions.show', $devotion) }}" class="inline-flex items-center gap-2 text-emerald-600 font-bold transition-all hover:gap-3">
                                            Read More
        ...
        <div class="mx-auto max-w-5xl">
            <div class="relative overflow-hidden rounded-3xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-600 via-teal-500 to-emerald-600 p-12 text-center shadow-2xl">
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('subscribe') }}" method="POST" class="mx-auto max-w-md space-y-4">
                        @csrf
                    ...
                    @if(session('subscriber_success'))
                        <div class="mt-6 rounded-xl border-2 border-white/50 bg-white/20 backdrop-blur-sm p-4">
                            <p class="text-white font-semibold">{{ session('subscriber_success') }}</p>
                        </div>
                    @endif
                </div>
            </div>
