        ...
        <div class="mx-auto max-w-4xl">
            ...
            <nav class="mb-8">
                <ol class="flex flex-wrap items-center gap-2 text-sm text-gray-600">
                    ...
                    <li>
                        <a href="{{ route('devotions.index') }}" class="transition-colors hover:text-emerald-600">Devotions</a>
                    </li>
                    <li>
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    ...
                    <li class="max-w-xs truncate font-medium text-teal-600">{{ $devotion->title }}</li>
                </ol>
            </nav>
            ...
            @if($devotion->scripture_reference)
                <div class="mb-6 flex items-center gap-3 rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 px-6 py-4 shadow-lg">
                    ...
                    <span class="text-lg font-bold text-emerald-800">{{ $devotion->scripture_reference }}</span>
                </div>
            @endif
            ...
            <div class="flex flex-wrap items-center gap-4">
                @if($devotion->author)
                    <div class="flex items-center gap-3 rounded-xl border-2 border-gray-200 bg-white px-4 py-3 shadow-sm">
                        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-lg font-bold text-white shadow-md">
                            {{ strtoupper(substr($devotion->author, 0, 1)) }}
                        </div>
                        <div>
                            <div class="text-xs text-gray-500">Written by</div>
                            <div class="font-bold text-gray-900">{{ $devotion->author }}</div>
                        </div>
                    </div>
                @endif
                <div class="flex items-center gap-2 rounded-xl border-2 border-gray-200 bg-white px-4 py-3 shadow-sm">
                    ...
                    <span class="font-semibold text-gray-700">{{ $devotion->created_at->format('F d, Y') }}</span>
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    @if($devotion->featured_image)
                        <div class="overflow-hidden rounded-3xl shadow-2xl border-4 border-white">
                            <img src="{{ asset('storage/' . $devotion->featured_image) }}" alt="{{ $devotion->title }}" class="w-full h-auto">
                        </div>
                    @endif
                    ...
                    @if($devotion->scripture_reference)
                        <div class="overflow-hidden rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 shadow-lg">
                            ...
                            <div class="p-6">
                                <p class="text-lg font-bold text-emerald-700">{{ $devotion->scripture_reference }}</p>
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="overflow-hidden rounded-2xl border-2 border-emerald-100 bg-gradient-to-br from-emerald-50 to-teal-50 shadow-lg">
                            ...
                            <div class="p-4 space-y-4">
                                @foreach($relatedDevotions as $related)
                                    <a href="{{ route('devotions.show', $related) }}" class="group block rounded-xl border-2 border-white bg-white p-4 shadow-sm transition-all hover:shadow-md hover:-translate-y-1">
                                        <div class="flex gap-3">
                                            @if($related->featured_image)
                                                <img src="{{ asset('storage/' . $related->featured_image) }}" alt="{{ $related->title }}" class="h-16 w-16 flex-shrink-0 rounded-lg object-cover">
                                            @else
        ...
        <div class="mx-auto max-w-5xl">
            <div class="relative overflow-hidden rounded-3xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-600 via-teal-500 to-emerald-600 p-12 text-center shadow-2xl">
                ...
                <div class="relative">
                    ...
                    <a href="{{ route('devotions.index') }}" class="inline-flex items-center gap-3 rounded-xl bg-white px-8 py-4 font-bold text-emerald-700 shadow-xl transition-all hover:shadow-2xl hover:bg-gray-50 hover:-translate-y-1">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                        </svg>
