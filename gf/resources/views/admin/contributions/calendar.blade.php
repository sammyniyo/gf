...
@section('title', 'Contributions Calendar')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- View Toggle -->
                    <a href="{{ route('admin.contributions.index') }}" class="px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl hover:shadow-lg transition-all">
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach($calendarData as $index => $memberData)
                            <tr class="border-b border-gray-200 hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 transition-all {{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                <!-- Member Name Column (Sticky) -->
                                <td class="px-4 py-3 sticky left-0 {{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }} z-10 border-r border-gray-200">
                                    <div class="flex items-center gap-2">
                                        <div class="flex-1">
                                            <p class="font-bold text-gray-900 text-sm flex items-center gap-2">
                                                {{ $memberData['name'] }}
                                                @if($memberData['has_award'])
                                                    <span class="text-xl">{{ $memberData['award_emoji'] }}</span>
                                                @endif
                                            </p>
                                            <div class="flex items-center gap-2 mt-1">
                                                <span class="px-2 py-0.5 rounded-full text-xs font-bold
                                                    {{ $memberData['category'] === 'student' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                                                    {{ strtoupper($memberData['category']) }}
                                                </span>
                                                <span class="text-xs text-gray-500 font-medium">{{ number_format($memberData['target']) }} RWF/mo</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                @foreach(range(1, 12) as $month)
                                    <td class="px-2 py-3 text-center">
                                        @if($memberData['months'][$month]['paid'])
                                            <div class="relative group">
                                                ...
                                                <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20">
                                                    Paid: {{ number_format($memberData['months'][$month]['amount']) }} RWF
                                                </div>
                                            </div>
                                        @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                            <div class="h-full rounded-full transition-all duration-500 {{ $memberData['completion'] >= 100 ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-gradient-to-r from-blue-500 to-indigo-600' }}"
                                                 style="width: {{ $memberData['completion'] }}%"></div>
                                        </div>
                                        <span class="text-sm font-bold {{ $memberData['completion'] >= 100 ? 'text-green-600' : 'text-gray-700' }}">
                                            {{ $memberData['completion'] }}%
                                        </span>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="p-6 bg-gradient-to-br from-green-50 to-emerald-100 rounded-2xl border-2 border-green-200 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-4xl font-black text-green-700 mt-2">{{ $fullyPaidMembers }}</p>
                    </div>
            ...
            <div class="p-6 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl border-2 border-blue-200 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-4xl font-black text-blue-700 mt-2">{{ $partiallyPaidMembers }}</p>
                    </div>
            ...
            <div class="p-6 bg-gradient-to-br from-rose-50 to-red-100 rounded-2xl border-2 border-rose-200 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-4xl font-black text-rose-700 mt-2">{{ $notPaidMembers }}</p>
                    </div>
            ...
            <div class="p-6 bg-gradient-to-br from-purple-50 to-violet-100 rounded-2xl border-2 border-purple-200 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-4xl font-black text-purple-700 mt-2">{{ $totalMembers }}</p>
                    </div>
