...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <span class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-1.5 text-xs font-semibold text-slate-600">
                Total: <span class="text-slate-900"><?php echo e($contacts->total()); ?></span>
            </span>
            <?php if($contacts->where('is_read', false)->count() > 0): ?>
                <span class="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50/70 px-3 py-1.5 text-xs font-semibold text-rose-700 animate-pulse">
                    ...
                    Unread: <span class="text-rose-900"><?php echo e($contacts->where('is_read', false)->count()); ?></span>
                </span>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="flex gap-2">
        ...
        <button class="filter-tab rounded-xl px-4 py-2 text-sm font-semibold text-slate-700 bg-white border border-slate-200 transition hover:bg-slate-50" data-filter="unread">
            Unread (<?php echo e($contacts->where('is_read', false)->count()); ?>)
        </button>
        <button class="filter-tab rounded-xl px-4 py-2 text-sm font-semibold text-slate-700 bg-white border border-slate-200 transition hover:bg-slate-50" data-filter="read">
            Read (<?php echo e($contacts->where('is_read', true)->count()); ?>)
        </button>
    </div>
    ...
    <div class="space-y-4">
        <?php $__empty_1 = true; $__currentLoopData = $contacts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contact): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="message-card glass-card overflow-hidden transition-all <?php echo e(!$contact->is_read ? 'border-l-4 border-indigo-400' : ''); ?>"
                data-status="<?php echo e($contact->is_read ? 'read' : 'unread'); ?>">
                <div class="p-5">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <!-- Left Side - Sender Info -->
                        <div class="flex items-start flex-1 gap-4">
                            <div class="flex h-12 w-12 items-center justify-center rounded-xl text-lg font-bold <?php echo e($contact->is_read ? 'bg-slate-200 text-slate-600' : 'bg-indigo-100 text-indigo-600'); ?>">
                                <?php echo e(substr($contact->name, 0, 1)); ?>

                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2.5 flex-wrap">
                                    <h3 class="text-base font-semibold text-slate-900"><?php echo e($contact->name); ?></h3>
                                ...
                                <div class="flex flex-wrap gap-3 mt-2">
                                    <a href="mailto:<?php echo e($contact->email); ?>" class="flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800">
                                ...
                                <div class="p-3 mt-3 rounded-lg bg-slate-50 border border-slate-200">
                                    ...
                                    <p class="mt-1 text-sm font-semibold text-slate-900"><?php echo e($contact->subject ?? 'No subject'); ?></p>

                                    <?php if($contact->message ?? false): ?>
                                        <p class="mt-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">Message Preview</p>
                                        <p class="mt-1 text-sm text-slate-700 line-clamp-2"><?php echo e(Str::limit($contact->message, 150)); ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-start lg:items-end gap-3">
                            <div class="text-left lg:text-right">
                                <p class="text-xs font-semibold text-slate-500"><?php echo e($contact->created_at->format('M d, Y')); ?></p>
                                <p class="text-xs text-slate-400"><?php echo e($contact->created_at->format('h:i A')); ?></p>
                                <p class="mt-0.5 text-xs font-semibold text-indigo-600"><?php echo e($contact->created_at->diffForHumans()); ?></p>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <?php if(!$contact->is_read): ?>
                                    <form action="<?php echo e(route('admin.contacts.read', $contact)); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                                ...
                                <?php else: ?>
                                    <form action="<?php echo e(route('admin.contacts.unread', $contact)); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('PATCH'); ?>
...
<?php echo $__env->make('admin.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\samsh\Documents\gf\gf\resources\views/admin/contacts/index.blade.php ENDPATH**/ ?>