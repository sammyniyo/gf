        ...
        <div class="glass-card px-5 py-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900">{{ $registrations->total() }}</p>
                </div>
        ...
        <div class="glass-card px-5 py-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900">{{ $registrations->sum('tickets') }}</p>
                </div>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-slate-900">{{ $registration->name }}</div>
                                        <div class="text-xs text-slate-500">{{ $registration->email }}</div>
                                        @if($registration->phone)
                                            <div class="text-xs text-slate-400">{{ $registration->phone }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-900">{{ $registration->event->title ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($registration->total_amount > 0)
                                    <span class="text-sm font-semibold text-emerald-600">{{ number_format($registration->total_amount) }} RWF</span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <code class="text-xs font-mono text-slate-600 bg-slate-100 px-2 py-1 rounded">{{ $registration->registration_code }}</code>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-900">{{ $registration->created_at->format('M d, Y') }}</div>
                                <div class="text-xs text-slate-500">{{ $registration->created_at->format('h:i A') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.registrations.destroy', $registration) }}" method="POST" class="inline"
