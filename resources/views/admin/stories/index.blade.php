...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.stories.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-slate-800">
    ...
    <div class="glass-card p-4">
        <form action="{{ route('admin.stories.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Search</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search stories..." class="w-full rounded-lg border-slate-300 focus:border-slate-400 focus:ring-slate-400 text-sm">
                        </div>
            <div>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('admin.stories.index') }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                    ...
                                    <img src="{{ asset('storage/' . $story->featured_image) }}" alt="{{ $story->title }}" class="w-12 h-12 rounded-lg object-cover">
                                            @else
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.stories.edit', $story) }}" class="text-sm font-semibold text-slate-900 hover:text-indigo-600">{{ Str::limit($story->title, 40) }}</a>
                                                    @if($story->is_featured)
                                        <svg class="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                                    @endif
                                            </div>
                                        </td>
                            <td class="px-6 py-4"><span class="inline-flex items-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-semibold text-indigo-700">{{ $categories[$story->category] ?? ucfirst($story->category) }}</span></td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $story->author ? $story->author->name : 'N/A' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600">{{ number_format($story->views_count ?? 0) }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $story->reading_time ?? 0 }} min</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $story->published_at ? $story->published_at->format('M d, Y') : 'Not published' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.stories.edit', $story) }}" class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</a>
                                    <form method="POST" action="{{ route('admin.stories.destroy', $story) }}" onsubmit="return confirm('Delete this story?');">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <td colspan="9" class="px-6 py-12 text-center">
                                ...
                                <a href="{{ route('admin.stories.create') }}" class="mt-4 inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                    Create New Story
                                            </a>
                                        </td>
