            ...
            <div class="inline-flex items-center gap-3 p-2 bg-gray-100 rounded-xl">
                ...
                @foreach($categories as $key => $label)
                    <a href="{{ route('resources.index', ['category' => $key]) }}"
                       class="inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-semibold transition-all
                       {{ $selectedCategory === $key ? 'bg-white text-emerald-600 shadow-md' : 'text-gray-600 hover:text-gray-900' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
                    ...
                    <div class="relative overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-lg transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 hover:border-pink-300">
                        ...
                        <div class="relative h-96 overflow-hidden bg-gradient-to-br from-pink-50 to-gray-100">
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
                                <h3 class="text-xl font-bold">{{ $uniform->title }}</h3>
                                @if($uniform->description)
                                    <p class="mt-2 text-sm text-white/90">{{ $uniform->description }}</p>
                                @endif
                            </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            @foreach($items as $resource)
                                <div class="group relative overflow-hidden rounded-xl border-2 border-gray-200 bg-white p-6 shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-emerald-300">
                                    ...
                                    <div class="mb-4">
                                        @if($resource->isImage())
                                            <img src="{{ $resource->file_url }}" alt="{{ $resource->title }}" class="w-full h-40 object-cover rounded-lg">
                                    ...
                                    <!-- Content -->
                                    <div class="mb-4">
                                        <h3 class="text-lg font-bold text-gray-900 mb-2 group-hover:text-emerald-600 transition-colors">{{ $resource->title }}</h3>
                                    ...
                                    <div class="flex items-center justify-between text-xs text-gray-500 mb-4">
                                        <span class="font-semibold">{{ strtoupper($resource->file_type) }}</span>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <a href="{{ route('resources.download', $resource) }}"
                                           class="{{ ($resource->isPdf() || $resource->isImage()) ? '' : 'flex-1' }} inline-flex items-center justify-center gap-2 rounded-lg border-2 border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition-all hover:border-emerald-500 hover:text-emerald-600 hover:shadow-lg">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                            </svg>
                                            Download
